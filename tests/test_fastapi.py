...
@pytest.mark.asyncio
async def test_openid_config_success(make_mock_async_client, discovery):
    mock_async_client = make_mock_async_client(MockResponse(json=discovery))
    oidc_auth = OIDCAuth("openid_url", "openid_url/.well-known/openid-configuration", "id", "secret")
...
@pytest.mark.asyncio
async def test_fetch_openid_config_failure(make_mock_async_client, discovery):
...
@pytest.mark.asyncio
async def test_userinfo_success_with_mock():
    oidc_auth = OIDCAuth("openid_url", "openid_url/.well-known/openid-configuration", "id", "secret")
...
def test_oidc_auth_initialization_default_extractor():
    oidc_auth = OIDCAuth("openid_url", "openid_url/.well-known/openid-configuration", "id", "secret")
    assert isinstance(
...
@pytest.mark.asyncio
async def test_authenticate_success(make_mock_async_client, discovery):
    mock_async_client = make_mock_async_client(MockResponse(json=discovery))
...
async def test_authenticate_bypassable_request(make_mock_async_client, discovery):
    ...
    with mock.patch("oauth2_lib.fastapi.AsyncClient", return_value=mock_async_client):
        ...
        oidc_auth = OIDCAuthMock("openid_url", "openid_url/.well-known/openid-configuration", "id", "secret")
...
@pytest.mark.asyncio
async def test_authenticate_token_extraction_failure(make_mock_async_client, discovery):
    mock_async_client = make_mock_async_client(MockResponse(json=discovery))
    with mock.patch("oauth2_lib.fastapi.AsyncClient", return_value=mock_async_client):
        request = mock.MagicMock(spec=Request)
        request.headers = {}
